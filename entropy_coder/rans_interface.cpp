...
std::string CdfParamsAwareEncoderDecoder::encode(const std::vector<int32_t> &symbols, const std::vector<int32_t> &indexes) {
    std::vector<RansSymbol> _syms;

    // backward loop on symbols from the end;
    for (size_t i = 0; i < symbols.size(); ++i) {
        const int32_t cdf_idx = indexes[i];
        const auto *cdf = cdfs + cdf_idx * cdfs_2nddim_size;
...
std::vector<int32_t> CdfParamsAwareEncoderDecoder::decode(const std::string &encoded, const std::vector<int32_t> &indexes) {
    ...
    for (int i = 0; i < static_cast<int>(indexes.size()); ++i) {
        ...
        const auto cdf_end = cdf + cdfs_sizes[cdf_idx];
...
void CdfParamsAwareEncoderDecoder::set_stream(std::string encoded) {
    _stream = std::move(encoded);
...
std::vector<int32_t> CdfParamsAwareEncoderDecoder::decode_stream(const std::vector<int32_t> &indexes) {
    std::vector<int32_t> output(indexes.size());
...
void CdfParamsAwareEncoderDecoder::release_stream() {
    _stream = "";
}